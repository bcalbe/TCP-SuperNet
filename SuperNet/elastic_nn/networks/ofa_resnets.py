...
import time
sys.path.append('./')
from SuperNet.elastic_nn.modules.dynamic_layers import (
    DynamicConvLayer,
    DynamicLinearLayer,
)
from SuperNet.elastic_nn.modules.dynamic_layers import (
...
from SuperNet.utils.layers import IdentityLayer, ResidualBlock
from SuperNet.networks import ResNets
from SuperNet.utils import make_divisible, val2list, MyNetwork
...
class OFAResNets(ResNets):
    def __init__(
        ...
        #n_blck_list 决定每个block有多少个 DynamicResNetBottleneckBlock
        n_block_list = [
...
if __name__ == "__main__":
    ...
    net.set_max_net()
    max_subnet = net.get_active_subnet()
    max_time = []
    min_time = []
    for i in range(100):
    ...
    #print("max time:mean {}, std {}".format(np.mean(max_time), np.std(max_time)))
    pass
